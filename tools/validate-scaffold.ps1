...
$constants = Get-Content -LiteralPath (Join-Path $ProjectRoot 'src/constants.lua') `
  -Raw -Encoding UTF8
if ($manifest.version -ne '0.34.4') {
  throw "manifest version must be 0.34.4"
}
...
if ($constants -notmatch 'MOD_VERSION\s*=\s*"0\.34\.4"') {
  throw "constants MOD_VERSION must match manifest version 0.34.4"
}
